...
function isToolDisabled(toolName) {
  const disabled = atom.config.get("pulsar-mcp.disabledTools") || [];
...
function handleToolsList(id) {
  ...
  const enabledTools = mcpTools.filter((t) => !isToolDisabled(t.name));
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === "GET" && pathname === "/tools") {
        ...
        const enabledTools = allTools.filter((t) => !isToolDisabled(t.name));
